...
* RunPod account + $10
* **Network Volume (20GB):** Console -> Storage -> `+ New volume` -> `Network volume` -> `h3-ref2va-q4` 20GB (tied to one datacenter, e.g. EU-RO-1). Global Volumes are **Pods-only** (Beta) — Serverless uses Network Volumes.

#### 1) Put model on Network Volume (one-time, 3 min via Pod)
```powershell
# Console -> Pods -> Deploy -> Community -> RTX 4090 -> Attach Volume h3-ref2va-q4 (Network, 20GB) -> Connect
mkdir -p /runpod-volume/models/diffusion_models /runpod-volume/models/unet
pip install -q huggingface_hub
# GGUF MUST be in diffusion_models/ (UnetLoaderGGUF with unet_gguf alias only scans diffusion_models/ per darkcoal-qwen-fast)
...
ls -lh /runpod-volume/models/diffusion_models/  # must show 11.6G!
```
...
4. **Storage:** Attach `h3-ref2va-q4` **Network volume** (must be same datacenter as workers, e.g. EU-RO-1). Mounts at `/runpod-volume` via `extra_model_paths.yaml`.
...
* **Network Volume:** 20GB * $0.07 = $1.40/mo (same region as workers)
